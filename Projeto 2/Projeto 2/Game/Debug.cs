...
namespace IPCA.Monogame
{
    public class Debug
    {
        ...
        public void DrawFullRectangle(SpriteBatch spriteBatch,RectangleF rectangle,Color color)
        {
            Point tl = Camera.Position2Pixels( new Vector2(rectangle.Left, rectangle.Top)).ToPoint();
            Point size = Camera.Meter2Pixels(rectangle.Size).ToPoint();



            spriteBatch.Draw(_pixel, new Rectangle(tl,size), color);
        }
